...
Fecha Fecha::operator ++ (int) { Fecha f(*this); *this += 1; return f; }
...
bool Fecha::operator < (Fecha &fecha) noexcept { 
	bool menor = true;
...
Fecha::operator const char*() { return this->a_cadena(); }

char* Fecha::a_cadena(){
	//char* mes;
	//char* dia;
	using namespace std;
	static char* result = new char[50];
	...
	strftime(result, 50, "%A %d de %B %Y" , tiempo_descompuesto);
	return result;
